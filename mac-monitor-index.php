    ...
    <section class="host-block" id="host-${sid}">
      ...
      <div class="gauges">
        ...
        <div class="gauge">
          ...
          <div class="sub" id="gpu-temp-${sid}" style="color:var(--muted);font-size:11px">–</div>
...
function updateGauges(sid, latest, now) {
  ...
  const cpu = +latest.cpu, gpu = +latest.gpu, gpu_temp = latest.gpu_temp, ram = +latest.ram_percent;
  ...
  if (document.getElementById('gpu-temp-' + sid)) document.getElementById('gpu-temp-' + sid).textContent = (gpu_temp != null ? gpu_temp + '°C' : '–');
